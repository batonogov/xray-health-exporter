...
Установите `RUN_ONCE=true` для выполнения одного цикла проверок и выхода — удобно для CI, скриптов и отладки. В этом режиме экспортёр:

1. Загружает конфигурацию и резолвит подписки.
2. Запускает Xray-инстансы для всех туннелей.
3. Выполняет ровно одну проверку на каждый туннель (параллельно).
4. Выводит все метрики в **stdout** в Prometheus text-exposition формате.
5. Останавливает все туннели и завершается.
